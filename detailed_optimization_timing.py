...
def create_detailed_timing_optimization():
    ...
    for run_idx in range(timing_iterations):
        ...
        timer = PerformanceTiming(
            f"OptimizationRun_{run_idx+1}", enable_gpu_timing=True
        )
    ...
    sorted_sections = sorted(
        avg_timings.items(), key=lambda x: x[1]["avg_duration"], reverse=True
    )
    ...
    print("=" * 80)
    print(
        f"{'Section':<35} {'Avg (ms)':<10} {'Std (ms)':<10} {'Count':<8} {'Total %':<10}"
    )
    print("-" * 80)

    total_avg_time = sum(
        data["avg_duration"] for _, data in sorted_sections if "full_optimization" in _
    )
    if total_avg_time == 0:
    ...
    for section_name, data in sorted_sections:
        ...
        percentage = (
            (data["avg_duration"] / total_avg_time * 100) if total_avg_time > 0 else 0
        )

        print(
            f"{section_name:<35} {avg_ms:<10.2f} {std_ms:<10.2f} {data['count']:<8} {percentage:<10.1f}"
        )
    ...
    iteration_sections = [
        ...
        if "iteration" in name or "dia_" in name or "gradient" in name
    ]
        ...
        for section in timer._sections.values():
            if section.memory_transfers:
                ...
                total_transfer_size += sum(
                    t["size_mb"] for t in section.memory_transfers
                )
    ...
    print(f"\n=== Final Summary ===")
    print(
        f"Overall optimization time: {overall_avg*1000:.1f}ms ± {overall_std*1000:.1f}ms"
    )
    print(f"Estimated FPS: {1/overall_avg:.1f}")
    print(
        f"Target 15 FPS requires: {1000/15:.1f}ms (current: {overall_avg*1000:.1f}ms)"
    )
