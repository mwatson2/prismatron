...
def debug_kernel_behavior():
    ...
    test_block_255 = cp.full((float32_tensor.block_size, float32_tensor.block_size), 255, dtype=cp.uint8)
    test_block_128 = cp.full((float32_tensor.block_size, float32_tensor.block_size), 128, dtype=cp.uint8)
    ...
    target_255 = cp.full((1, float32_tensor.height, float32_tensor.width), 255, dtype=cp.uint8)
    ...
    print(f"Target dtype: {target_255.dtype}, value: {target_255[0, 0, 0]} (should be 255)")
    ...
    print(f"  LED 1: {expected_128_normalized * block_pixels:.2f} (block_pixels * ~0.502)")
