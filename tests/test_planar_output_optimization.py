...
def test_planar_output_optimization():
    ...
    # Load the 2624 LED synthetic patterns from production patterns directory
    pattern_file = Path("/mnt/prismatron/patterns/synthetic_2624_uint8.npz")
    ...
    assert (
        results_identical
    ), f"Results should be identical, got max_diff={np.max(np.abs(result_old_cpu - result_new_cpu)):.6f}"
...
def test_frame_optimizer_integration():
    ...
    # Load the same data from production patterns directory
    pattern_file = Path("/mnt/prismatron/patterns/synthetic_2624_uint8.npz")
    ...
    assert (
        ATb_result.shape == expected_shape
    ), f"ATb result has wrong shape: {ATb_result.shape}, expected: {expected_shape}"
