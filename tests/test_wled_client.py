...
from src.consumer.wled_client import WLEDClient, WLEDConfig, DDPFlags, DDPDataType
from src.const import LED_COUNT, DDP_HEADER_SIZE, DDP_MAX_DATA_PER_PACKET
...
class TestWLEDClient(unittest.TestCase):
    ...
    def _setup_mock_query_response(self, mock_socket, status_json=None):
        ...
        response_data = struct.pack('>BBBBLH', 
                                  DDPFlags.VER1 | DDPFlags.REPLY, 0, 0, 251, 0, len(status_json))
    ...
    def test_fragmentation_calculation(self):
        ...
        expected_packets = (3000 + DDP_MAX_DATA_PER_PACKET - 1) // DDP_MAX_DATA_PER_PACKET
    ...
    def test_connection(self, mock_socket_class):
        ...
        self._setup_mock_query_response(mock_socket, '{"name":"Test WLED","ver":"0.14.0","leds":{"count":100}}')
    ...
    def test_ddp_packet_structure(self):
        ...
        with patch('socket.socket') as mock_socket_class:
            ...
            flags, sequence, data_type, dest_id, offset, data_length = struct.unpack('>BBBBLH', sent_packet[:10])
            
            # Verify header fields
            self.assertEqual(flags & DDPFlags.VER1, DDPFlags.VER1)
            self.assertEqual(flags & DDPFlags.PUSH, DDPFlags.PUSH)  # Last packet should have PUSH
...
class TestWLEDQueryResponse(unittest.TestCase):
    ...
    def test_malformed_json_response(self, mock_socket_class):
        ...
        response_data = struct.pack('>BBBBLH', 
                                  DDPFlags.VER1 | DDPFlags.REPLY, 0, 0, 251, 0, len(malformed_json))
    ...
    def test_refresh_wled_status(self, mock_socket_class):
        ...
        response_data = struct.pack('>BBBBLH', 
                                  DDPFlags.VER1 | DDPFlags.REPLY, 0, 0, 251, 0, len(updated_json))
    ...
    def test_incomplete_data_packet(self, mock_socket_class):
        ...
        incomplete_data = struct.pack('>BBBBLH', 
                                    DDPFlags.VER1 | DDPFlags.REPLY, 0, 0, 251, 0, 100)
    ...
    def test_oversized_data_length(self, mock_socket_class):
        ...
        oversized_header = struct.pack('>BBBBLH', 
                                     DDPFlags.VER1 | DDPFlags.REPLY, 0, 0, 251, 0, 50000)
    ...
    def test_zero_length_data(self, mock_socket_class):
        ...
        zero_data_packet = struct.pack('>BBBBLH', 
                                     DDPFlags.VER1 | DDPFlags.REPLY, 0, 0, 251, 0, 0)
...
class TestWLEDPersistentRetry(unittest.TestCase):
    ...
    def test_persistent_retry_success(self, mock_socket_class, mock_sleep):
        ...
        response_data = struct.pack('>BBBBLH', 
                                  DDPFlags.VER1 | DDPFlags.REPLY, 0, 0, 251, 0, len(test_json))
