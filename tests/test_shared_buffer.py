...
from src.const import FRAME_CHANNELS, FRAME_HEIGHT, FRAME_WIDTH, METADATA_DTYPE
from src.core.shared_buffer import (
    ...
    get_frame_timing_info,
    get_shared_control_state,
)
...
def producer_process(
    ...
    num_frames: int,
    pattern_value: int,
    delay_per_frame: float = 0.01,
    write_timeout: float = 1.0,
    test_resolution: bool = False,
    ...
    try:
        ...
        # Keep producer alive long enough for consumer to read
        time.sleep(max(2.0, num_frames * delay_per_frame * 2))
...
def consumer_process(
    ...
    try:
        ...
        while frames_received < expected_frames and (time.time() - start_time) < 15.0:
            ...
            if buffer_info is not None:
                ...
                sample_array = get_buffer_array(buffer_info)[:100, :100, :]
                if np.all(sample_array == expected_pattern):
                    frames_received += 1
    ...
    if test_timing:
        result = frames_received >= expected_frames * 0.6
        sys.exit(0 if result else 1)
    else:
        # For non-timing tests, accept if we got at least some frames
        # Ring buffers provide latest frames, not necessarily every frame
        # For backpressure tests, getting 30% of frames is reasonable
        min_expected = max(1, expected_frames // 3)
        result = frames_received >= min_expected
        sys.exit(0 if result else 1)
...
class TestProducerConsumerSubclasses(unittest.TestCase):
    ...
    def test_consumer_subclass_basic_functionality(self):
        ...
        self.assertIsNotNone(display_info.buffer)
        self.assertIsNotNone(display_info.metadata)
        self.assertIsNotNone(display_info.frame_id)
