...
class TestPrismatronImageBasic:
    ...
    def test_initialization_planar(self):
        ...
        data = np.random.randint(0, 256, (3, 100, 80), dtype=np.uint8)  # (3, width, height)
    ...
    def test_initialization_interleaved(self):
        ...
        data = np.random.randint(0, 256, (100, 80, 3), dtype=np.uint8)  # (width, height, 3)
...
class TestFormatConversions:
    ...
    def setup_method(self):
        """Setup test image."""
        self.test_data = np.random.randint(0, 256, (3, 50, 40), dtype=np.uint8)  # (3, width, height)
    ...
    def test_as_interleaved(self):
        ...
        assert interleaved.shape == (50, 40, 3)  # (width, height, 3)
    ...
    def test_as_normalized_float(self):
        ...
        assert norm.shape == (50, 40, 3)  # (width, height, 3)
    ...
    def test_as_normalized_planar_float(self):
        ...
        assert norm.shape == (3, 50, 40)  # (3, width, height)
        assert norm.dtype == np.float32
        assert np.all(norm >= 0.0) and np.all(norm <= 1.0)
...
class TestImageOperations:
    ...
    def setup_method(self):
        """Setup test image."""
        # Create a simple test pattern (3, width, height) = (3, 100, 80)
...
class TestQualityMetrics:
    ...
    def setup_method(self):
        ...
        # Create slightly different image - same dimensions as img1/img2 (3, 50, 40)
...
class TestAnalysisMethods:
    ...
    def setup_method(self):
        ...
        data[1, :40, :] = 100        # Green half
        data[2, 60:, 50:] = 200      # Blue corner
        self.img = PrismatronImage(data, "planar")
...
class TestCameraBackend:
    ...
    def test_camera_capture(self, mock_cv2):
        ...
        assert frame.shape == (480, 640, 3)  # OpenCV standard (height, width, 3)
