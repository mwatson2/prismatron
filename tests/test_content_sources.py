...
from src.producer.content_sources.base import (
    ContentSource, ContentType, ContentStatus, ContentInfo, 
    ContentSourceRegistry, FrameData
)
...
class MockContentSource(ContentSource):
    ...
    def get_next_frame(self) -> Optional[FrameData]:
        ...
        test_array = np.full((self.content_info.height, self.content_info.width, 3), 
                           color_value, dtype=np.uint8)
...
@unittest.skipUnless(PILLOW_AVAILABLE or OPENCV_AVAILABLE, 
                     "PIL or OpenCV required for image tests")
