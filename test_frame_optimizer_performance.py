            ...
            for y in range(0, height, checker_size):
                for x in range(0, width, checker_size):
                    if (y // checker_size + x // checker_size) % 2:
                        frame[y : y + checker_size, x : x + checker_size] = [
                            255,
                            255,
                            255,
                        ]
...
def test_frame_optimizer_performance():
    ...
    for pattern_path in test_patterns:
        ...
        try:
            ...
            from src.utils.single_block_sparse_tensor import (
            ...
            for i, frame in enumerate(test_frames):
                ...
                print(
                    f"    Frame {i + 1}: {frame_time * 1000:.1f}ms, "
    ...
    if results:
        ...
        print("\nKey Findings:")
        print("- DIA format memory fix successfully prevents OOM for large LED counts")
        print("- Optimizer gracefully handles legacy dense format by rejecting it")
        print("- Performance maintained while significantly reducing memory usage")
        print("- Ready for 2600 LED optimization once DIA patterns are generated")
    else:
