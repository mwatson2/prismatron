...
def create_test_data(led_count=512, use_small_test=True):
    ...
    batch_ata = BatchSymmetricDiagonalATAMatrix.from_symmetric_diagonal_matrix(
        symmetric_ata,
        batch_size=8  # For 8-frame batch processing
    )
...
def main():
    ...
    try:
        ...
        if success_small:
            ...
            print("SUMMARY")
            print("=" * 80)
            print(f"Small test (208 LEDs): {'PASS' if success_small else 'FAIL'} - {speedup_small:.2f}x speedup, {error_small*100:.6f}% error")
            print(f"Large test (512 LEDs): {'PASS' if success_large else 'FAIL'} - {speedup_large:.2f}x speedup, {error_large*100:.6f}% error")
        else:
