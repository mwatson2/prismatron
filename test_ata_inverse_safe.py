...
def test_dia_matrix_extraction(data):
    ...
    try:
        ...
        logger.log_success("create_dia_matrix", f"LED count: {dia_matrix.led_count}, bandwidth: {dia_matrix.bandwidth}")
...
def test_ata_inverse_computation(dia_matrix):
    ...
    try:
        ...
        logger.log_success("compute_ata_inverse", f"Shape: {ata_inverse.shape}, successful: {successful_inversions}/3")
...
def test_specific_pattern_file(pattern_file_path):
    ...
    # Test ATA inverse computation
    ata_inverse, successful_inversions, condition_numbers, avg_condition_number = test_ata_inverse_computation(dia_matrix)
