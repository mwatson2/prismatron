...
def analyze_mixed_dia_timing():
    ...
    print(
        f"Per-iteration work: ~{per_iteration_time:.1f}ms × {mean_iterations:.1f} = {per_iteration_time * mean_iterations:.1f}ms"
    )
    ...
    print(
        f"Theoretical minimum: {theoretical_min:.1f}ms ({mean_iterations:.1f} iterations × 2 calls × {raw_3d_kernel_time:.1f}ms)"
    )
