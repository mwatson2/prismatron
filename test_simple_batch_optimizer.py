...
def test_simple_batch_optimizer():
    ...
    batch_ata = BatchSymmetricDiagonalATAMatrix.from_symmetric_diagonal_matrix(
        symmetric_ata, 
        batch_size=8  # Match the frame batch size
    )
    ...
    try:
        ...
        print(f"✓ Batch optimizer completed successfully!")
        ...
        if relative_error < 0.05:  # 5% tolerance for TF32 differences
            print(f"  ✓ Small differences as expected (TF32 precision)")
        else:
            print(f"  ✗ Large differences suggest algorithmic issue")
