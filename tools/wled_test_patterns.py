...
class WLEDTestRunner:
    ...
    def run_animated_pattern(self, pattern_name: str, speed: float = 1.0, 
        ...
        try:
            ...
            while self.running:
                ...
                if frame_count % (30 * 5) == 0:  # Every ~5 seconds at 30 FPS
                    ...
                    print(f"Status: {frame_count} frames, {fps:.1f} FPS, "
...
Examples:
  ...
  %(prog)s wave --speed 0.5 --frequency 3.0
    ...
    # Global options
    parser.add_argument('--host', default=WLED_DEFAULT_HOST,
                        help=f'WLED controller hostname/IP (default: {WLED_DEFAULT_HOST})')
    parser.add_argument('--port', type=int, default=WLED_DEFAULT_PORT,
    ...
    parser.add_argument('--persistent-retry', action='store_true',
    ...
    solid_parser.add_argument('--color', nargs=3, type=int, metavar=('R', 'G', 'B'),
                              default=[255, 255, 255],
    ...
    rainbow_parser.add_argument('--speed', type=float, default=1.0,
                                 help='Cycle speed in Hz (default: 1.0)')
    
    # Animated rainbow pattern
    animated_parser = subparsers.add_parser('animated-rainbow', help='Animated rainbow pattern')
    animated_parser.add_argument('--speed', type=float, default=1.0,
                                  help='Animation speed in Hz (default: 1.0)')
    animated_parser.add_argument('--width', type=float, default=1.0,
                                  help='Rainbow width as fraction of array (default: 1.0)')
    
    # Wave pattern
    wave_parser = subparsers.add_parser('wave', help='Sine wave pattern')
    wave_parser.add_argument('--speed', type=float, default=1.0,
                              help='Wave speed in Hz (default: 1.0)')
    wave_parser.add_argument('--frequency', type=float, default=2.0,
    ...
    try:
        if args.pattern == 'test':
            ...
            success = runner.run_animated_pattern('rainbow_cycle', 
                                                speed=args.speed, 
                                                duration=args.duration)
            
        elif args.pattern == 'animated-rainbow':
            success = runner.run_animated_pattern('animated_rainbow',
                                                speed=args.speed,
                                                duration=args.duration,
                                                width=args.width)
            
        elif args.pattern == 'wave':
            success = runner.run_animated_pattern('wave_pattern',
                                                speed=args.speed,
                                                duration=args.duration,
