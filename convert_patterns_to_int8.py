...
def convert_patterns_to_int8(input_path: str, output_path: str):
    ...
    print(
        f"Original values range: [{float(cp.min(float32_mixed_tensor.sparse_values)):.6f}, {float(cp.max(float32_mixed_tensor.sparse_values)):.6f}]"
    )
    ...
    for key, value in patterns_data.items():
        if key == "mixed_tensor":
            ...
            print(f"Replaced mixed_tensor with int8 version")
        else:
            # Copy existing data
    ...
    print(
        f"Verified values range: [{int(cp.min(verify_mixed_tensor.sparse_values))}, {int(cp.max(verify_mixed_tensor.sparse_values))}]"
    )
