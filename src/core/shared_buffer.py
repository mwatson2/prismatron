...
from ..const import FRAME_WIDTH, FRAME_HEIGHT, FRAME_CHANNELS, FRAME_SIZE, BUFFER_COUNT, METADATA_DTYPE
...
class FrameRingBuffer:
    ...
    def _get_write_index(self) -> int:
        ...
        assert self._initialized and self._control_array is not None, "Buffer must be initialized and have control array"
    ...
    def _get_buffer_timestamp(self, buffer_index: int) -> float:
        ...
        assert self._initialized and self._control_array is not None, "Buffer must be initialized and have control array"
        assert 0 <= buffer_index < self.buffer_count, f"Buffer index {buffer_index} out of range [0, {self.buffer_count})"
        return self._control_array[4 + buffer_index]
    ...
    def get_write_buffer(self, timeout: float = 1.0, presentation_timestamp: Optional[float] = None, 
                        source_width: Optional[int] = None, source_height: Optional[int] = None) -> Optional[Dict]:
        """
    ...
    def wait_for_ready_buffer(self, timeout: float = 1.0) -> Optional[Dict]:
        """
...
class FrameProducer(FrameRingBuffer):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Each metadata record: [presentation_timestamp, source_width, source_height, capture_timestamp]
            ...
            # Layout: [write_idx, read_idx, frame_counter, min_consumed_frame, timestamp0, timestamp1, timestamp2]
        ...
        try:
            while (time.time() - start_time) < timeout:
                with self._lock:  # Protect against concurrent access
                    ...
                    if (read_idx == -1 or write_idx != read_idx) and self._is_write_buffer_available(write_idx, current_frame):
    ...
    def advance_write(self) -> bool:
        ...
        try:
            with self._lock:  # Atomic operation across all updates
                ...
                logger.debug(f"Advanced write: buffer {write_idx} -> {next_write_idx}, "
    ...
    def get_producer_stats(self) -> Dict[str, any]:
        ...
        base_status.update({
            ...
            'is_producer': True,
            'write_buffer_available': self._is_write_buffer_available(
                int(self._control_array[0]), int(self._control_array[2])
            ) if self._initialized else False
        })
        return base_status
    ...
    def can_write_frame(self) -> bool:
        ...
        try:
            with self._lock:
                ...
                return ((read_idx == -1 or write_idx != read_idx) and 
                       self._is_write_buffer_available(write_idx, current_frame))
        except Exception:
            return False
...
def get_buffer_utilization(ring_buffer: FrameRingBuffer) -> Dict[str, float]:
    """
