"""
Test Renderer for LED Display System.

This module provides a test renderer that uses the mixed sparse tensor to calculate
...
class TestRendererConfig:
    ...
    window_name: str = "Prismatron Test Renderer"
...
class TestRenderer:
    """
    Test renderer that uses mixed sparse tensor to render LED values to a display window.

    This renderer takes LED values, uses the mixed sparse tensor to calculate Ax
    ...
    def __init__(
        ...
        config: Optional[TestRendererConfig] = None,
    ):
        """
        ...
        self.config = config or TestRendererConfig()
        ...
        logger.info(f"Test renderer initialized with display size {self.display_size}")

    def start(self) -> bool:
        """
        Start the test renderer display window.
        ...
        try:
            if self.is_running:
                logger.warning("Test renderer already running")
                return True
            ...
            self.display_thread = threading.Thread(target=self._display_loop, name="TestRenderer", daemon=True)
            self.display_thread.start()

            logger.info("Test renderer started")
            return True

        except Exception as e:
            logger.error(f"Failed to start test renderer: {e}")
            return False
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            logger.info("Stopping test renderer...")
            self.is_running = False
            ...
            logger.info("Test renderer stopped")

        except Exception as e:
            logger.error(f"Error stopping test renderer: {e}")
    ...
    def _display_loop(self) -> None:
        ...
        logger.info("Test renderer display loop started")
        ...
        logger.info("Test renderer display loop ended")
    ...
    def reset_statistics(self) -> None:
        ...
        self.total_render_time = 0.0
        self.current_fps = 0.0
        logger.info("Test renderer statistics reset")
    ...
    def __enter__(self):
        ...
        if self.start():
            ...
            raise RuntimeError("Failed to start test renderer")
...
def create_test_renderer_from_pattern(
    pattern_file_path: str, config: Optional[TestRendererConfig] = None
) -> Optional[TestRenderer]:
    """
    Create a test renderer from a diffusion pattern file.
    ...
    Returns:
        TestRenderer instance or None if failed to load
    """
    try:
        # Load pattern data
        ...
        # Create renderer
        renderer = TestRenderer(mixed_tensor, config)

        logger.info(f"Created test renderer from pattern: {pattern_file_path}")
        return renderer

    except Exception as e:
        logger.error(f"Failed to create test renderer from pattern: {e}")
        return None
...
def test_renderer_with_solid_color(renderer: TestRenderer, r: int, g: int, b: int) -> bool:
    """
    Test the renderer with a solid color pattern.

    Args:
        renderer: Test renderer instance
    ...
    try:
        ...
        # Render
        return renderer.render_led_values(led_values)
...
def test_renderer_with_gradient(renderer: TestRenderer) -> bool:
    """
    Test the renderer with a gradient pattern.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test LED renderer")
    ...
    # Create config
    config = TestRendererConfig(display_scale=args.scale, show_stats=not args.no_stats)

    # Create renderer
    renderer = create_test_renderer_from_pattern(args.pattern_file, config)
    if not renderer:
    ...
    try:
        ...
        logger.info("Test renderer started. Press 'q' or ESC in window to exit.")
        ...
        for r, g, b in test_patterns:
            ...
            test_renderer_with_solid_color(renderer, r, g, b)
        ...
        if renderer.is_running:
            ...
            test_renderer_with_gradient(renderer)
            time.sleep(3)
