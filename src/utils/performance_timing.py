...
# GPU support - conditionally imported and commented out for systems without CuPy
try:
    # import cupy as cp
    # GPU_AVAILABLE = True
    GPU_AVAILABLE = False  # Commented out for systems without CuPy
    cp = None
except ImportError:
    GPU_AVAILABLE = False
    cp = None
...
class TimingSectionContext:
    ...
    def __init__(self, timing_instance: 'PerformanceTiming', section_name: str, 
                 **kwargs):
...
class PerformanceTiming:
    ...
    def _gpu_sync(self):
        ...
        if self.enable_gpu_timing and cp is not None:
            # cp.cuda.runtime.deviceSynchronize()  # Commented out for systems without CuPy
            pass
            
    def _create_gpu_events(self):
        """Create GPU timing events if available."""
        if self.enable_gpu_timing and cp is not None:
            # return cp.cuda.Event(), cp.cuda.Event()  # Commented out for systems without CuPy
            return None, None
        return None, None
        
    def _calculate_automatic_flops(self, operation_type: str, 
    ...
    def start(self, section_name: str, read: int = 0, written: int = 0, 
              flops: int = 0, use_gpu_events: bool = False,
              operation_type: Optional[str] = None,
              shape_info: Optional[Dict[str, Tuple[int, ...]]] = None) -> bool:
        """
        ...
        def _start_impl():
            ...
            if use_gpu_events:
                section.gpu_start_event, section.gpu_end_event = self._create_gpu_events()
                if section.gpu_start_event:
                    # section.gpu_start_event.record()  # Commented out for systems without CuPy
                    pass
    ...
    def stop(self, section_name: str, read: int = 0, written: int = 0, 
             flops: int = 0) -> bool:
        """
        ...
        def _stop_impl():
            ...
            if section.use_gpu_events and section.gpu_end_event:
                # section.gpu_end_event.record()  # Commented out for systems without CuPy
                # cp.cuda.runtime.deviceSynchronize()  # Commented out for systems without CuPy
                # section.gpu_duration = cp.cuda.get_elapsed_time(
    ...
    def _format_section_report(self, section: TimingSection, 
                             include_percentages: bool = False,
                             total_time: float = 0.0) -> List[str]:
    ...
    def log(self, logger: logging.Logger, include_percentages: bool = False,
    ...
    def _log_children(self, logger: logging.Logger, parent: TimingSection,
                     include_percentages: bool, total_time: float):
    ...
    def export_csv(self, filename: str) -> bool:
        ...
        def _export_impl():
            ...
            with open(filename, 'w', newline='') as csvfile:
                fieldnames = [
                    'section_name', 'duration', 'occurrence_count',
                    'read_bytes', 'written_bytes', 'flops',
                    'memory_bandwidth_gbps', 'gflops_per_second',
                    'gpu_duration', 'depth'
                ]
