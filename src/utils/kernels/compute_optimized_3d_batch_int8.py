"""
Compute-optimized 3D batch CUDA kernel for uint8 inputs with FP32 output.

This module implements the uint8 batch version of the optimized CUDA kernel for LED optimization operations,
specifically the batched A^T @ B matrix multiplication where both A and B are uint8 with FP32 output.
"""
...
# 3D batch CUDA kernel for uint8 inputs with shared memory optimization
COMPUTE_OPTIMIZED_3D_BATCH_INT8_KERNEL = r"""
...
extern "C" __global__
void compute_optimized_3d_batch_transpose_dot_product_int8_kernel(
    ...
    const unsigned char* target_batch,   // (batch_frames, channels, height, width) - int8
    float* result,                       // (batch_frames, batch_size, channels) or (batch_frames, channels, batch_size) - fp32
    ...
    extern __shared__ unsigned char led_pattern[];  // Dynamic shared memory allocation
    ...
    int thread_sum_int = 0;  // Use int for accumulation to avoid overflow, then convert to fp32
    ...
    for (int row_offset = 0; row_offset < rows_per_thread; row_offset++) {
        ...
        for (int col = 0; col < block_size; col += 4) {
            ...
            uchar4 led_vec = *reinterpret_cast<const uchar4*>(
                &led_pattern[led_row_offset + col]);

            // Compute dot product
            thread_sum_int += dot_int8(led_vec, target_vec);
        }
    }
    ...
    if (warp.thread_rank() == 0) {
        ...
        result[result_idx] = sum / (255.0f * 255.0f);
    }
}
"""
...
def get_compute_optimized_3d_batch_int8_kernel():
    """Get the compiled compute-optimized 3D batch CUDA kernel for uint8 inputs."""
    if "compute_optimized_3d_batch_int8" not in _kernel_cache:
        logger.info("Compiling compute-optimized 3D batch int8 CUDA kernel...")

        # Compile the kernel
        kernel = cp.RawKernel(
            COMPUTE_OPTIMIZED_3D_BATCH_INT8_KERNEL,
            "compute_optimized_3d_batch_transpose_dot_product_int8_kernel",
        )
        _kernel_cache["compute_optimized_3d_batch_int8"] = kernel

        logger.info("Compute-optimized 3D batch int8 CUDA kernel compiled successfully")

    return _kernel_cache["compute_optimized_3d_batch_int8"]


def cuda_transpose_dot_product_3d_batch_compute_optimized_int8(
    ...
    """
    3D Batch Compute-optimized CUDA kernel wrapper for batched A^T @ B operation with uint8 inputs.
    ...
    and reuses it across all computations within that block. Uses integer arithmetic
    internally then converts to fp32 with proper scaling.

    Args:
        sparse_values: Dense LED blocks, shape (channels, batch_size, block_size, block_size), uint8
        block_positions: Block positions, shape (channels, batch_size, 2)
        target_batch: Target frames, shape (batch_frames, channels, height, width), uint8
    ...
    # Get compiled kernel
    kernel = get_compute_optimized_3d_batch_int8_kernel()
    ...
    logger.debug("3D Batch Compute-optimized int8 CUDA kernel launch:")
    ...
    logger.debug(f"  Target batch shape: {target_batch.shape} (uint8)")
    ...
    # Each block stores one LED pattern (block_size x block_size uint8)
    shared_mem_size = block_size * block_size  # 1 byte per uint8
    ...
    kernel(
        ...
        (
            ...
            target_batch.ravel(),  # Flatten 4D batch input to 1D uint8
