...
__global__ void batch8_symmetric_block_pair_multiply_wmma_experimental(
    ...
    int max_block_diag,          // Maximum block diagonal index (bandwidth-based)
    ...
    for (int block_col_j = 0; block_col_j < led_blocks; block_col_j++) {
        ...
        // Direct access using implicit diagonal ordering: diag_idx = col - row
        int top_diag_idx = block_col_j - block_row_top;
        if (top_diag_idx >= 0 && top_diag_idx < max_block_diag) {
            // Block exists in upper diagonal
            top_block_ptr = block_data +
                (channel_idx * max_block_diag * led_blocks * 16 * 16 +
                 top_diag_idx * led_blocks * 16 * 16 +
                 block_row_top * 16 * 16);
            use_transpose_top = false;
        } else if (top_diag_idx < 0) {
            // Check symmetric position: (block_col_j, block_row_top)
            int sym_diag_idx = block_row_top - block_col_j;
            if (sym_diag_idx > 0 && sym_diag_idx < max_block_diag) {
                // Block exists in transpose position
                top_block_ptr = block_data +
                    (channel_idx * max_block_diag * led_blocks * 16 * 16 +
                     sym_diag_idx * led_blocks * 16 * 16 +
                     block_col_j * 16 * 16);
        ...
        if (block_row_bottom < led_blocks) {
            int bottom_diag_idx = block_col_j - block_row_bottom;
            if (bottom_diag_idx >= 0 && bottom_diag_idx < max_block_diag) {
                // Block exists in upper diagonal
                bottom_block_ptr = block_data +
                    (channel_idx * max_block_diag * led_blocks * 16 * 16 +
                     bottom_diag_idx * led_blocks * 16 * 16 +
                     block_row_bottom * 16 * 16);
                use_transpose_bottom = false;
            } else if (bottom_diag_idx < 0) {
                // Check symmetric position: (block_col_j, block_row_bottom)
                int sym_diag_idx = block_row_bottom - block_col_j;
                if (sym_diag_idx > 0 && sym_diag_idx < max_block_diag) {
                    // Block exists in transpose position
                    bottom_block_ptr = block_data +
                        (channel_idx * max_block_diag * led_blocks * 16 * 16 +
