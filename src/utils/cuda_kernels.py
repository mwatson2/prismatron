...
# 3D Multi-channel CUDA kernel for int8 inputs with fp32 output
...
__device__ __forceinline__ uchar4 load_uchar4_unaligned(const unsigned char* ptr) {
    // Load 4 uint8 values, handling any alignment
...
void compute_optimized_3d_transpose_dot_product_int8_kernel(
    ...
    // Row-based processing optimization: each thread processes whole rows
    // Requires: block_size % 4 == 0 (for vectorized uchar4 access)
    // Grid: (batch_size, channels) - one block per (LED, channel) combination
    // Block: (32 threads, 1, 1) - each thread processes block_size/32 complete rows
    ...
    for (   ;
            thread_row < max_thread_row;
            thread_row++, target_idx += width - block_size) {
        // Process all columns in this row with vectorized int8 operations
        for (int col = 0;
             col < block_size;
             col += 4, target_idx += 4, sparse_idx += 4) {
            uchar4 sparse_vec = *reinterpret_cast<const uchar4*>(&sparse_values[sparse_idx]);
            uchar4 target_vec = load_uchar4_unaligned(&target_3d[target_idx]);
...
def get_compute_optimized_3d_int8_kernel():
    """Get the compiled compute-optimized 3D int8 CUDA kernel for planar input."""
    if "compute_optimized_3d_int8" not in _kernel_cache:
        logger.info("Compiling compute-optimized 3D int8 CUDA kernel...")
        ...
        _kernel_cache["compute_optimized_3d_int8"] = kernel

        logger.info("Compute-optimized 3D int8 CUDA kernel compiled successfully")

    return _kernel_cache["compute_optimized_3d_int8"]
